...
import {toMarkingFormat, getDateData} from '../../interface';
...
describe('ExpandableCalendar', () => {
  ...
  describe('Knob', () => {
    ...
    it('should not close expandable header on day press when closeOnDayPress is false', () => {
      const aDriver = new ExpandableCalendarDriver(
        testIdExpandableCalendar,
  ...
  describe('CalendarList updates', () => {
    ...
    describe('arrow press', () => {
      ...
      it.each([[Direction.LEFT], [Direction.RIGHT]])(
        ...
        (direction: Direction) => {
          ...
          const expectedDate = today
            .clone()
            .addMonths(direction === Direction.RIGHT ? 1 : -1)
            .setDate(1);
          ...
          expect(onMonthChange).toHaveBeenCalledWith(getDateData(expectedDate), UpdateSources.PAGE_SCROLL);
        }
      );
      ...
      it(`should call onDateChanged and onMonthChanged for first day in initial month when changing to initial month`, () => {
        ...
        expect(onMonthChange).toHaveBeenNthCalledWith(2, getDateData(expectedDate), UpdateSources.PAGE_SCROLL);
      });
      ...
      it(`should navigate 6 months ahead and back successfully`, () => {
        ...
        expect(onMonthChange).toHaveBeenNthCalledWith(6, getDateData(expectedFutureDate), UpdateSources.PAGE_SCROLL);
        ...
        expect(onMonthChange).toHaveBeenNthCalledWith(12, getDateData(expectedDate), UpdateSources.PAGE_SCROLL);
      });
    });
  });
  ...
  describe('WeekCalendar updates', () => {
    ...
    describe('arrow press', () => {
      ...
      it.each([[Direction.LEFT], [Direction.RIGHT]])(
        `should call onDateChanged to next week first day when pressing %s arrow`,
        direction => {
      ...
      it('should call onMonthChange when new week first day is in a different month', () => {
        ...
        const diff =
          Math.ceil((endOfMonth.getUTCDate() + 1 - today.getUTCDate()) / 7) +
          (today.getUTCDay() > endOfMonth.getUTCDay() ? 1 : 0);
  ...
  describe.skip('today button', () => {
    ...
    it('should call onMonthChanged with initial month when today button pressed while initial date is not in initial month', () => {
      ...
      driver.render();
      driver.pressOnTodayButton();
      expect(onMonthChange).toHaveBeenCalledWith(getDateData(today), UpdateSources.TODAY_PRESS);
    });
  });
});
